...
function onPlayerReady(event) {
	$(event.target.f).closest('.videoEmbed').removeClass('buffering').addClass('ready');
}
...
$(document).ready(function() {
	...
	youtubeEmbeds();
	retinaImgs();
	$('.vimeoEmbed').find('img').on('click', function(){
		if ($(this).closest('div').hasClass('not-ready')) {
			var frame = $(this).siblings('iframe');
			$(this).closest('div').addClass('buffering');
			frame.attr('src', $(this).siblings('iframe').data('src'));
			frame.load(function(){
				vimeoEmbeds(this);
			});
        ...
        function ready(player_id) {
            ...
            function onPlay() {
                froogaloop.addEvent('play', function(data) {
                	//Pause other videos
                	$('.vimeoEmbed:not(.not-ready)').find('.vimeo:not(#' + data +')').each(function(){
                		var frame = $f(this);
                		frame.api('pause');
                	});
                	//Play this video
