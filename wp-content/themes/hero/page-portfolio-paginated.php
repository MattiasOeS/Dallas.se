			...
			<div id="content" class="fullProjects clearfix full grid">									
				...
				<div id="projects" class="clearfix">		
					...
					<?php if ($page_skills) : // if there are a limited number of skills set ?>
						...
						<?php else: ?>
							<?php $skill = $skills[0]; ?>
							<?php $s = get_term_by( 'name', trim(htmlentities($skill)), 'tag'); ?>
							<?php if($s) { $skill_slugs = $s->slug; } ?>
						<?php endif; 	

						query_posts( 'tag='.$skill_slugs.'&post_type=project&posts_per_page=3&paged='.$paged );
					...
					<?php  while (have_posts()) : the_post(); ?>
						...
						global $p;				
						$p = "";
						$skills = get_the_terms( $post->ID, 'tag');
						if ($skills) {
